...
@REM xcopy %~dp0teablacklist C:\"Program Files (x86)"\teablacklist /h /y /i /e /v
@REM xcopy C:\"Program Files (x86)"\teablacklist\Minecraft.exe C:\teablacklist\ /h /y /i /e

@reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\teablack_cmd_list" /v DisplayName /t REG_SZ /d "茶紅自製功能選單" /f
@reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\teablack_cmd_list" /v DisplayVersion /t REG_SZ /d "1.6" /f
